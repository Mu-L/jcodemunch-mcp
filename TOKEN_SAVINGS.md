...
average (27.7x)** against a grep-and-read agent across 15 tasks and 3 pinned
repositories. Quote the range, not the average alone: per-query results span
7.4x to 84.6x, so the aggregate on its own overstates the low end. The larger
99.6% / 235.3x figure is against a *read-every-file* baseline, which is a
