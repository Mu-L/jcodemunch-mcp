...
- **Anonymous savings telemetry.** The server periodically sends a random anonymous ID plus aggregate token-savings counters to the project's public community meter. No code, no file paths, no repo names, no PII — counters only, and **no per-tool detail** — the payload is exactly `{delta, total, anon_id}`. The sender is a single background daemon thread that starts lazily on the first share (never at import, and never if you have opted out), so a plain import has no background side effect. Opt out with `share_savings: false` in `config.jsonc` or `JCODEMUNCH_SHARE_SAVINGS=0`; redirect the endpoint with `JCODEMUNCH_TELEMETRY_URL`.
- **One-time tool-surface notice.** On server start, if your `tool_surface` is not the surface a new install would receive today, one WARNING line is written to the **log channel** naming what a switch would save on your install and the two commands that accept or silence it. It is emitted **once per install** and never again: a marker at `~/.code-index/surface_offer_state.json` records that it was said (a timestamp, the surface, the version, and the pid and transport of the process that said it — nothing else). Nothing is sent anywhere, nothing is changed, and your config is not written; the notice exists because `tool_surface` is written once at install time and no upgrade can revisit it, so an install created before the current default would otherwise never learn there is a choice. Silence it permanently with `jcodemunch-mcp config set surface_offer_seen true`, or delete `~/.code-index/` to erase the marker.
