...
| Racket            | `.rkt`, `.rktl`, `.rktd`                        | tree-sitter-racket              | function (define/define-syntax/define-syntax-rule), constant (value define/define-values), class (struct/module+/class), type (define-type/define-signature/define-generics), method (define/public etc.) | — | `;;` and `#\| \|#` preceding comments | Homoiconic grammar; custom walker. A macro's own name is indexed, but **names created by *invoking* a macro are not** (`(define-lgetter second 2)` defines a real exported `second` that cannot be seen statically). `#lang at-exp racket/base` works; `.scrbl` is deliberately not claimed. A `provide` rename is not represented, so `(rename-out [greet say-hello])` indexes `greet`. Struct accessors are synthesised: `(struct posn (x y))` yields `posn`, `posn?`, `posn-x`, `posn-y` (plus setters when mutable), all pointing at the struct form. `require` edges resolve for both collection paths and string paths (a string require is relative to the importing file). A project's own defining macros (`defstep`, `defstudy`) can be declared via `racket_definition_forms` in `.jcodemunch.jsonc`. Needs no Racket install. Measured against Racket's expander: 86.5% of definitions found, 0 names that do not exist — see `benchmarks/racket_fidelity/` |
