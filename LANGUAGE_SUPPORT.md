...
| Racket            | `.rkt`, `.rktl`, `.rktd`                        | tree-sitter-racket              | function (define/define-syntax/define-syntax-rule), constant (value define/define-values), class (struct/module+/class), type (define-type/define-signature/define-generics), method (define/public etc.) | — | `;;` and `#\| \|#` preceding comments | Homoiconic grammar; custom walker. **Macro-defined names are invisible** — `(define-lgetter second 2)` binds a real exported name no static parser can see. Measured 86.2% of human-written module bindings, 0 fabricated names (`benchmarks/racket_fidelity/`). `rename-out` not represented; `.scrbl` deliberately unsupported |
