...
- **Older releases (1.108.292 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8264 passed, 17 skipped, **0 failed** (1.108.295) **+ `uv run ruff check src/` clean**, measured on the settled tree before the release commit. ⚠ Reconciled: **8281 TOTAL**, unchanged from .294's 8281 — the four items shipped here were merged before the bump, so their +16/+8/+6 already landed. ⚠ 3.13 CI-env reproduce via `uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q`: **8264 passed / 17 skipped, the same 8281 TOTAL AND the same skip split**, run SEQUENTIALLY after the local suite. ⚠ Prior (1.108.294): 8234 passed, 17 skipped, **0 failed**. ⚠ Prior (1.108.293): 8163 passed, 17 skipped, **0 failed**. ⚠⚠ **Two false-green mechanisms live in the rotated-out history and BOTH reported `exit code 0`** (see `ISSUE-HISTORY.md`, Tests-line appendix): **pytest-xdist is invisible to a bare `python -m pytest`**, so the flags are rejected, NOTHING is collected and the harness reports success — use `uv run pytest` whenever xdist flags are passed; and **a trailing `| tail` swallows pytest's exit status**. **Write to a log and echo the exit code BEFORE any pipe.** [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠⚠ **Compare TOTALS, never passed counts** — across interpreters or releases. ⚠ Local suite is `uv run pytest tests/ -n 4 --dist loadfile`; the 3.13 reproduce runs SEQUENTIALLY after it — two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier.
- **Python:** >=3.10
