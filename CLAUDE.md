...
- **Older releases (1.108.291 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8234 passed, 17 skipped, **0 failed** (1.108.294) **+ `uv run ruff check src/` clean**, measured on the settled tree before the release commit. ⚠ Reconciled: **8251 TOTAL** vs .293's 8180 — **+5** `test_integrity_check_cost.py` and **+66** from #535, and #535's own CI reported **8246 collected**, which is 8251 minus those 5 exactly. ⚠ 3.13 CI-env reproduce via `uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q`: **8234 passed / 17 skipped, the same 8251 TOTAL AND the same skip split**, run SEQUENTIALLY after the local suite. ⚠ Prior (1.108.293): 8163 passed, 17 skipped, **0 failed**. ⚠ Prior (1.108.292): 8161 passed, 17 skipped, **0 failed**. ⚠⚠ **Two false-green mechanisms live in the rotated-out history and BOTH reported `exit code 0`** (see `ISSUE-HISTORY.md`, Tests-line appendix): **pytest-xdist is invisible to a bare `python -m pytest`**, so the flags are rejected, NOTHING is collected and the harness reports success — use `uv run pytest` whenever xdist flags are passed; and **a trailing `| tail` swallows pytest's exit status**. **Write to a log and echo the exit code BEFORE any pipe.** [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠⚠ **Compare TOTALS, never passed counts** — across interpreters or releases. ⚠ Local suite is `uv run pytest tests/ -n 4 --dist loadfile`; the 3.13 reproduce runs SEQUENTIALLY after it — two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier.
- **Python:** >=3.10
