...
- **Older releases (1.108.294 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8315 passed, 17 skipped, **0 failed** (1.108.297) **+ `uv run ruff check src/` clean**, measured on the settled tree before the release commit. ⚠ **8332 TOTAL, unchanged from .296** — this release adds no tests, so it reproduces the prior run exactly. ⚠ 3.13 CI-env reproduce via `uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q`: **8315 passed / 17 skipped, the same 8332 TOTAL AND the same skip split**, run SEQUENTIALLY after the local suite. ⚠⚠ **The `PARSER_GENERATION` bump costs one re-parse per stale index and the suite pays it too**: local 186s -> **198s** (+6%), 3.13 558s -> **508s**. **The 3.13 run got FASTER, so do not read the local delta as the bump's cost** — one sample each, and the two move in opposite directions. ⚠ Prior (1.108.296): 8315 passed, 17 skipped, **0 failed**. ⚠ Prior (1.108.295): 8264 passed, 17 skipped, **0 failed**. ⚠⚠ **Two false-green mechanisms live in the rotated-out history and BOTH reported `exit code 0`** (see `ISSUE-HISTORY.md`, Tests-line appendix): **pytest-xdist is invisible to a bare `python -m pytest`**, so the flags are rejected, NOTHING is collected and the harness reports success — use `uv run pytest` whenever xdist flags are passed; and **a trailing `| tail` swallows pytest's exit status**. **Write to a log and echo the exit code BEFORE any pipe.** [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠⚠ **Compare TOTALS, never passed counts** — across interpreters or releases. ⚠ Local suite is `uv run pytest tests/ -n 4 --dist loadfile`; the 3.13 reproduce runs SEQUENTIALLY after it — two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier.
- **Python:** >=3.10
