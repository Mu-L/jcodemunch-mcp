...
- **Older releases (1.108.301 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8727 passed, 13 skipped, **0 failed** (1.108.304) **+ `uv run ruff check src/` clean**, measured on the settled tree after the bump and the rotation. ⚠ **8740 TOTAL, +21 over .303's 8719** — the 5 new fast-path tests plus 16 from parametrized cases; small because this release is one fix and one instrument. ⚠⚠ **Compare TOTALS, never passed counts, and NEVER a skip count across machines** — CI ubuntu skips 26 and windows 19 where this box skips 13, all pre-existing. I once read a CI/local skip gap as a new gate silently skipping and was WRONG; **the before/after delta on the SAME job is the only signal**. ⚠⚠ **A BACKGROUND-TASK BANNER SAYING "exit code 0" IS NOT A GREEN SUITE** — one run here reported exit 0 having never started pytest (`--timeout` plugin absent, argparse rejected the flag). **Redirect the exit code INTO the log (`{ pytest; echo "EXIT=$?"; } > log`) and grep that line**; the banner reports the shell's status, not the suite's. ⚠⚠ **A CONTRIBUTOR PR IS TRIAL-MERGED ONTO `main` AND RUN LOCALLY BEFORE THE MERGE, never read off the branch** — branch-green is not merged-green and the merge base moves every release. ⚠ 3.13 CI-env reproduce: **CLEAN** — 8721 passed, 19 skipped, **0 errors**, the SAME 8740 total (the skip gap is the pre-existing env difference, not a new gate). ⚠⚠ **A reproduce that ERRORS in `tests/test_sdist_exclusions.py` is NOT environmental noise** — that file is the sdist CREDENTIAL-LEAK guard (v0.2.6), and it errors at setup when the scratch venv must fetch the build backend with DNS blocked. Re-run it explicitly on the release tree; never wave it through. ⚠ Prior (1.108.303): 8706 passed, 13 skipped, **0 failed** (8719 total). ⚠ Prior (1.108.302): 8517 passed, 13 skipped, **0 failed** (8530 total). ⚠ Two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier, so the 3.13 reproduce runs AFTER the local suite, never beside it.
- **Python:** >=3.10
