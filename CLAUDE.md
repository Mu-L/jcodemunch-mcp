...
- **Older releases (1.108.289 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8161 passed, 17 skipped, **0 failed** (1.108.292) **+ `uv run ruff check src/` clean**, measured on the settled tree before the release commit. ⚠ Reconciled against .291's **8170 total**: **+8** `test_mcp_instructions.py` = **8178** exactly, so nothing else moved. ⚠ 3.13 CI-env reproduce via `uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q`: **8161 passed / 17 skipped, the same 8178 TOTAL AND the same skip split**, run SEQUENTIALLY after the local suite. ⚠⚠ **A 3.13 run started mid-edit came back 3 RED and every failure was `test_claude_md_rotation`** — the suite raced the rotation it gates, not the code. **A reproduce is only a reproduce against a SETTLED tree**; both runs above were re-done after the last edit. ⚠⚠ **That same run's background banner reported `exit code 0` while the log's own `EXIT=` line said 1.** The banner is not the verdict. [[a-trailing-command-hides-pytests-exit-code]] ⚠ Prior (1.108.291): 8153 passed, 17 skipped, **0 failed**. ⚠ Prior (1.108.290): 8105 passed, 17 skipped, **0 failed**. ⚠⚠ **Two false-green mechanisms live in the rotated-out history and BOTH reported `exit code 0`** (see `ISSUE-HISTORY.md`, Tests-line appendix): **pytest-xdist is invisible to a bare `python -m pytest`**, so the flags are rejected, NOTHING is collected and the harness reports success — use `uv run pytest` whenever xdist flags are passed; and **a trailing `| tail` swallows pytest's exit status**, so a run with a real failure reports 0. **Write to a log and echo the exit code BEFORE any pipe.** [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠ **Compare TOTALS, never passed counts**, across interpreters or across releases — a different skip split is normal and a whole absent subsystem is invisible from `N passed`. ⚠ Local suite is `uv run pytest tests/ -n 4 --dist loadfile`; the 3.13 reproduce runs SEQUENTIALLY after it — two full runs contend on the same `~/.code-index` process-lock scopes and that is the documented cause of .261's 47m outlier.
- **Python:** >=3.10
