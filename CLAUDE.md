...
- **Older releases (1.108.299 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8517 passed, 13 skipped, **0 failed** (1.108.302) **+ `uv run ruff check src/` clean**, measured on the settled tree after the bump and rotation. ⚠ **8530 TOTAL, +35 over .301's 8495** — decomposed: **9 from `tests/test_undeclared_table_guard.py`** (#555) and **26 from the Rust fidelity pair** (13 gating frozen-oracle data + 13 artifact-sync). ⚠⚠ **Compare TOTALS, never passed counts, and NEVER a skip count across machines** — CI ubuntu skips 26 and windows 19 where this box skips 13, all pre-existing. I read a CI/local skip gap as the new gate silently skipping and was WRONG; **the before/after delta on the SAME job is the only signal** (+26 passed, skips unchanged, on both platforms). ⚠ 3.13 CI-env reproduce: **CLEAN this time** — 8511 passed, 19 skipped, **0 errors**, the SAME 8530 total. ⚠⚠ **`.301`'s reproduce was NOT clean and the difference is the SCRATCH VENV's cache, not the tree**: `tests/test_sdist_exclusions.py` errors at setup when `uv build --sdist` must fetch the build backend and the sandbox blocks DNS. **That file is the sdist CREDENTIAL-LEAK guard (v0.2.6), so when it errors, re-run it explicitly on the release tree — never wave it through as environmental.** ⚠ Prior (1.108.301): 8482 passed, 13 skipped, **0 failed** (8495 total). ⚠ Prior (1.108.300): 8436 passed, 17 skipped, **0 failed** (8453 total). ⚠ Two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier, so the 3.13 reproduce runs AFTER the local suite, never beside it.
- **Python:** >=3.10
