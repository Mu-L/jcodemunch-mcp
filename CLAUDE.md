...
- **Older releases (1.108.295 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8379 passed, 17 skipped, **0 failed** (1.108.298) **+ `uv run ruff check src/` clean**, measured on the settled tree after the bump and rotation. ⚠ **8396 TOTAL, +64 over .297's 8332** — decomposed, not inferred: six `tests/test_refresh_corpus_unreadable.py` cases plus **58 from #548's Racket suite**. ⚠ The pre-merge tree measured 8338/+6 and that run is superseded; #548 merged AFTER the release commit, so the settled tree is the merge, not the bump. ⚠ 3.13 CI-env reproduce via `uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q`: **the SAME 8338 TOTAL AND the same skip split** (8321 passed / 17 skipped), run SEQUENTIALLY after the local suite; 262s local vs 568s sequential. ⚠⚠ **The documented 3.13 command FAILED TO START and the reason will recur: `uv` rebuilds the project into `.venv` and could not replace `Scripts/jcodemunch-mcp.exe`, which THIS SESSION'S live MCP server holds open** (`os error 5`, Access is denied). It is not a test failure and re-running does not clear it. Point the run at a throwaway env instead — `UV_PROJECT_ENVIRONMENT=<scratch> uv run --python 3.13 ...` — which also leaves the working `.venv` untouched. ⚠⚠ **The rotation gate fired mid-release AND WAS RIGHT**, on a silent no-op in my own patch script: deleting the `.295` line shifted every later index, so the `Older releases` edit landed on the Tests line and matched nothing. **An index captured before a deletion is stale after it**; the gate caught what a `replace()` returning unchanged text cannot. ⚠ Prior (1.108.297): 8315 passed, 17 skipped, **0 failed**. ⚠ Prior (1.108.296): 8315 passed, 17 skipped, **0 failed**. ⚠⚠ **Two false-green mechanisms live in the rotated-out history and BOTH reported `exit code 0`** (see `ISSUE-HISTORY.md`, Tests-line appendix): **pytest-xdist is invisible to a bare `python -m pytest`**, so the flags are rejected, NOTHING is collected and the harness reports success — use `uv run pytest` whenever xdist flags are passed; and **a trailing `| tail` swallows pytest's exit status**. **Write to a log and echo the exit code BEFORE any pipe.** ⚠ Both fired again this release: a background-task banner read `exit code 0` while the log's own `EXIT=` line read `1`. [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠⚠ **Compare TOTALS, never passed counts** — across interpreters or releases. ⚠ Local suite is `uv run pytest tests/ -n 4 --dist loadfile`; the 3.13 reproduce runs SEQUENTIALLY after it — two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier.
- **Python:** >=3.10
