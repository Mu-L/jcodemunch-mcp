...
- **Older releases (1.108.297 and earlier):** see `CHANGELOG.md`. The 1.108.182 entry ("a stall has a name and a ceiling", #375) and the 1.108.177-.181 #377 hardening arc are there in full.
- **Tests:** 8436 passed, 17 skipped, **0 failed** (1.108.300) **+ `uv run ruff check src/` clean**, measured on the settled tree after the bump and rotation. ⚠ **8453 TOTAL, +22 over .299's 8431** — decomposed: **15 from `tests/test_python_sibling_imports.py`** (#550, two of them the unreported `import `-in-prose crash and its now-logged swallow) plus **7 from `tests/test_call_outcome_contract.py`** (#551/#552, AST guards over the property). ⚠ 3.13 CI-env reproduce: the SAME 8453 TOTAL and the same skip split, run SEQUENTIALLY after the local suite via `UV_PROJECT_ENVIRONMENT=<scratch> uv run --python 3.13 --group dev --extra watch python -m pytest tests/ -q` — the plain form fails with `os error 5` while this session's live MCP server holds `.venv/Scripts/jcodemunch-mcp.exe` open, which is not a test failure and does not clear on a re-run. ⚠⚠ **The false-green fired AGAIN this release and was caught only by the echoed exit code**: a background-task banner read `exit code 0` while the log's own `EXIT=` line read `1`, on a run with a real failure. **Write to a log and echo the exit code BEFORE any pipe** — and the sibling mechanism, **pytest-xdist being invisible to a bare `python -m pytest`** (flags rejected, NOTHING collected, success reported), is why xdist flags always go through `uv run pytest`. Both live in `ISSUE-HISTORY.md`'s Tests-line appendix. [[pipes-and-missing-xdist-both-report-exit-zero]] ⚠ Prior (1.108.299): 8414 passed, 17 skipped, **0 failed** (8431 total). ⚠ Prior (1.108.298): 8379 passed, 17 skipped, **0 failed** (8396 total). ⚠⚠ **Compare TOTALS, never passed counts** — across interpreters or releases. ⚠ Two full runs contend on the same `~/.code-index` process-lock scopes, the documented cause of .261's 47m outlier, so the 3.13 reproduce runs AFTER the local suite, never beside it. ⚠⚠ **An index captured before a deletion is stale after it** — the rotation gate caught exactly that in .298's own patch script, on a `replace()` that returned unchanged text and raised nothing.
- **Python:** >=3.10
