...
**Status 2026-09-04 (inbound layer, `docs/inbound/`):** seven headless workflows put items 1-4, 7 and 17 in front of inbound issues and PRs as well as our own changes: triage labels and drafts, dependency PRs are graded by the gate's Floor lines and a full-corpus bench, an agent-authored fix is opened only after the failing test is proven red on `main` and the reviewer approves. No item below moves by it; the layer adds no gate and loosens none (POLICY 4.4 makes `harness/thresholds.json`, `retired.json`, the standard and `ARCHAEOLOGY.md` untouchable to every headless job). Open: `docs/inbound/FINDINGS.md`.
